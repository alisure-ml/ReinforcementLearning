...
class GameEnvironment:
    ...
    # 重置
    def reset(self):
        # 所有物体的size和intensity为1，hero的channel为2，goal的channel为1，fire的channel为0
        self.objects = []
...
class QNetwork:
    ...
    def __init__(self, environment, input_size, channel):
        ...
        conv_final_size = 512
        # 输入
        self.scalar_input = tf.placeholder(
            shape=[None, self.input_size * self.input_size * self.channel], dtype=tf.float32)
        # 变形为图片输入
        ...
        # 预测的Q值
        self.Q = tf.reduce_sum(tf.multiply(self.q_out, tf.one_hot(self.actions, environment.actions,
                                                                  dtype=tf.float32)), reduction_indices=1)
            ...
            while step < self.max_episode_length:
                ...
                if self.total_steps > self.pre_train_steps:
                    ...
                    if self.total_steps % self.update_freq == 0:
                        train_batch = self.my_buffer.sample(self.batch_size)  # state, action, reward, now_state, done

                        predict = self.sess.run(main_dqn.predict, feed_dict={
                            main_dqn.scalar_input: np.vstack(train_batch[:, 3])})  # now_state
                        q_out = self.sess.run(target_dqn.q_out, feed_dict={
                            target_dqn.scalar_input: np.vstack(train_batch[:, 3])})  # now_state
                        q_double = q_out[range(self.batch_size), predict]
                        q_target = train_batch[:, 2] + self.discount_factor * q_double  # reward

                        self.sess.run(main_dqn.optimizer, feed_dict={
                            main_dqn.scalar_input: np.vstack(train_batch[:, 0]),  # state
                            main_dqn.targetQ: q_target,
                            main_dqn.actions: train_batch[:, 1]  # action
                        })
